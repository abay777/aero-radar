...
export const MapPage: React.FC = () => {
  ...
  const { flightData, loading, error, path } = useSelector((state: RootState) => state.flightList);
  ...
  return (
    <>
      {error ? ( 
        <section>
             <p className="font-semibold text-red-400 animate-pulse text-xl text-center">😭sorry we found that Flight details is currently suspended due to high usage</p> 
        <MapContainer
      ...
      >
        ...
        />
        {flights && flights.map((plane: FormattedResponse) => (
